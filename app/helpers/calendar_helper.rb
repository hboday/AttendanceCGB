...
def calendar_for_next_30_days(notes={})
  today = Date.today
  start_date = (today - today.wday) # First Sunday of the current week
  end_date = start_date + 30

  last_sunday = start_date - 7
      ...
      content_tag(:div, class: 'd-flex justify-content-center w-100') do
        week.map.with_index do |day, day_index|
          if week_index == 0 && day_index < last_sunday.wday
            ...
            css_classes = ['text-center', day == today ? 'bg-primary text-white' : '']
            css_classes << 'prev-month' if day && day < start_date
            css_classes << 'next-month' if day && day > end_date
            content_tag(:div, class: 'p-2', style: 'width: calc(100% / 7)') do
              if day.present?
                note = notes[day.to_s] || ''
                render partial: 'shared/calendar_icon', locals: { date: day, note: note }
